...
object AMapUtil {
    ...
    fun addHeardAndEndForDriveStep(list: List<DriveStep>): List<DriveStep> {
        val driveSteps = ArrayList<DriveStep>()
        val driveStep = DriveStep()
        driveSteps.add(driveStep)
        for (drive in list) {
            driveSteps.add(drive)
        }
        driveSteps.add(driveStep)
        return driveSteps
    }

    fun addHeardAndEndForWalkStep(list: List<WalkStep>): List<WalkStep> {
        val walkSteps = ArrayList<WalkStep>()
        val walkStep = WalkStep()
